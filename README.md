...
#注意
在原始代码上修改的内容都用 WDD 做了标注，可以搜索WDD 快速找到修改的部分

#重点修改了
1) train.py

2) scene/dataset_readers.py
...
## CameraInfo类
   * 增加了保存关键帧的字段：keyframe:int
   * 增加保存每个关键帧mesh数据的字段

## SceneInfo类
   * 增加 keyframe_meshes 属性
   
## def readCamerasFromTransforms(...)函数
   * 增加了读取keyframe字段的代码
   * 增加根据keyfram
...
## create_from_pcd(...)函数
   * 增加了keyframe_meshes的输入参数和index的序列

## get_xyz(self)函数
   * 当前xyz填入的是loc_x,loc_y和loc_z
   * 需要根据三角形索引index和关键帧序号keyframe来获得正确的所属三角形顶点世界坐标
   * 之后根据所属三角形顶点世界坐标，和局部坐标，计算当前顶点的世界坐标

## 修改了很多使用

4) utils/camera_utils.py

## cameraList_from_camInfos()函数
   * 为返回值增加可keyframe
